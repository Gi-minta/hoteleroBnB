...
function Navbar({ onReservar }: { onReservar: () => void }) {
  ...
  const links = [
    ...
    tr("Ubicación", "Location"), tr("Galería", "Gallery"), tr("FAQ", "FAQ"), tr("Contacto", "Contact"),
  ]
  const ids = ["inicio", "habitaciones", "servicios", "ubicacion", "galeria", "faq", "contacto"]
    ...
    <section id="inicio" className="relative min-h-[130vh] flex items-end text-white overflow-hidden">
      ...
      <div className="relative z-20 max-w-6xl mx-auto px-6 pb-20 w-full">
        ...
        <p className="text-white/90 text-lg max-w-lg mb-6" style={{ fontFamily: "'Work Sans', sans-serif" }}>
          {t("hero", "description", "Apartamentos cómodos y totalmente equipados cerca a Unicentro, Laureles, Pueblito Paisa y los mejores lugares de la ciudad.", "Comfortable, fully equipped apartments near Unicentro, Laureles, Pueblito Paisa and the best places in the city.")}
        </p>
...
function Apartments({ t }: { t: Translate }) {
  const tr = useTr()
  const roomCards = [
    { eyebrow: tr("Apartamento 01", "Apartment 01"), name: tr("Apartamento Ejecutivo", "Executive Apartment"), desc: tr("Compacto y funcional, perfecto para viajeros de negocios o parejas. Cocina equipada y sala propia.", "Compact and functional, perfect for business travelers or couples. Equipped kitchen and private living room."), tags: [tr("1 habitación", "1 bedroom"), tr("2 personas", "2 guests"), tr("Cocina equipada", "Equipped kitchen")], images: ["/images/apt-ejecutivo.jpg", "/images/apt-ejecutivo-sala.jpg", "/images/apt-ejecutivo2.jpg"] },
    { eyebrow: tr("Apartamento 02", "Apartment 02"), name: tr("Apartamento Familiar", "Family Apartment"), desc: tr("Amplio y luminoso, con dos habitaciones y cocina con isla. Ideal para familias o grupos pequeños.", "Spacious and bright, with two bedrooms and an island kitchen. Ideal for families or small groups."), tags: [tr("2 habitaciones", "2 bedrooms"), tr("4 personas", "4 guests"), tr("Cocina con isla", "Island kitchen")], images: ["/images/apt-familiar1.jpg", "/images/apt-familiar2.jpg", "/images/apt-familiar-cocina.jpg"] },
  ]
...
function Services({ t }: { t: Translate }) {
  const tr = useTr()
  const items = [
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8V6a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v2"/></svg>`, title: tr("Habitaciones iluminadas", "Bright rooms"), desc: tr("Habitaciones con ventanas a la calle y amplios espacios, con baño privado o compartido. Camas dobles con posibilidad de camas sencillas adicionales.", "Rooms with street-facing windows and spacious layouts, with private or shared bathroom. Double beds with the option of additional single beds.") },
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5S5 13 5 15a7 7 0 0 0 7 7z"/></svg>`, title: tr("Baños privados y compartidos", "Private and shared bathrooms"), desc: tr("Tres baños privados y uno compartido, con servicio de agua fría para ducha y lavamanos.", "Three private bathrooms and one shared, with cold water service for shower and sink.") },
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2s2-.9 2-2V2"/><path d="M5 11v11"/><path d="M17 22V2c1.7 0 3 1.3 3 3v6h-3"/></svg>`, title: tr("Cocina compartida y equipada", "Shared, equipped kitchen"), desc: tr("Amplia cocina dotada con todos los electrodomésticos necesarios para la preparación de tus alimentos.", "A large kitchen fitted with all the appliances you need to prepare your meals.") },
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M17 8h1a3 3 0 0 1 0 6h-1"/><path d="M3 8h14v6a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4V8z"/><path d="M6 1v3"/><path d="M10 1v3"/><path d="M14 1v3"/></svg>`, title: tr("Desayuno y lavandería", "Breakfast and laundry"), desc: tr("Desayuno de 8 a 10 a.m. (paisa o liviano), tienda de confianza con mecato y bebidas, y servicio de lavandería.", "Breakfast from 8 to 10 a.m. (paisa or light), a trusted shop with snacks and drinks, and laundry service.") },
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="13" rx="2"/><path d="M8 3l4 4 4-4"/></svg>`, title: tr("Salón / comedor", "Lounge / dining room"), desc: tr("Salón de descanso con sala de televisión, sofás y mesas para compartir con la comunidad multicultural que nos visita.", "A lounge with a TV area, sofas and tables to share with the multicultural community that visits us.") },
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2M5 5l1.5 1.5M17.5 17.5 19 19M19 5l-1.5 1.5M6.5 17.5 5 19"/></svg>`, title: tr("Balcón", "Balcony"), desc: tr("Amplio balcón para un buen desayuno y un café en las tardes, con las brisas de los parques del barrio Conquistadores.", "A large balcony for a good breakfast and afternoon coffee, with the breezes from the parks of the Conquistadores neighborhood.") },
    ...
    { icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21V9l9-5 9 5v12"/><path d="M3 21h18"/><path d="M8 21v-6h8v6"/></svg>`, title: tr("Parqueadero de motos", "Motorcycle parking"), desc: tr("Parqueadero para motos en garaje privado; los vehículos se parquean en la calle bajo responsabilidad de los propietarios.", "Motorcycle parking in a private garage; cars are parked on the street at the owners' own risk.") },
  ]
  return (
...
function Location() {
  ...
  const places = [
    ...
    { name: "U. P. Bolivariana", time: "0.5 km", icon: "🎓", desc: tr("El barrio Conquistadores se desarrolló alrededor de la UPB.", "The Conquistadores neighborhood grew up around UPB."), img: "/images/upb.jpg" },
    ...
    <section id="ubicacion" className="py-20 bg-[#F1EBDD] dark:bg-[#1A251D]">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-ink-soft dark:text-[#A7B6AB] text-sm mt-2 max-w-2xl mx-auto">{tr("Estamos en el barrio residencial Conquistadores (sector Laureles–Estadio), con más de 60 años de historia alrededor de la UPB y lleno de parques y zonas verdes para disfrutar.", "We're in the residential Conquistadores neighborhood (Laureles–Estadio area), with over 60 years of history around UPB and full of parks and green spaces to enjoy.")}</p>
        </div>
